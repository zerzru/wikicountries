<?php
    require_once('engine/engine.admin.php');
...
<html>
    ...
    <body>
        ...
        <div class="created_article">
            ...
            <table class="table">
                ...
                <?php
                    if($administration=='TRUE') {
                        $request = mysqli_query($link, "SELECT * FROM banned WHERE status='Не рассмотрен'");
                        while($row = mysqli_fetch_assoc($request)) {
                            echo "<tr><td>{$row['login']}</td><td>{$row['id']}</td><td>{$row['date']}</td><td>{$row['ip']}</td><td>{$row['text']}</td>
                                          <form action='/engine/engine.admin.php?still={$row['id']}' method='post'><td><input name='still' type='submit' class='submit' value='Оставить'></form>
                                          <form action='/engine/engine.admin.php?unblock={$row['id']}&user={$row['login']}' method='post'><input name='unblock' type='submit' class='submit' value='Разблокировать'></form></td></tr>";
                        }
            ...
            <form action="/engine/engine.admin.php">
